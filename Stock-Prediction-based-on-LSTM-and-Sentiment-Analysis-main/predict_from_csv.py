...
# 最终评估区长度：二选一
EVAL_DAYS = 126       # 约半年交易日；设为 None 则使用 EVAL_RATIO
EVAL_RATIO = 0.20     # 若 EVAL_DAYS=None，则评估最后 20%
...
# =========================================================
def evaluate_final_period(model, x_scaler, meta, df: pd.DataFrame, device: str):
    ...
    if EVAL_DAYS is not None:
        n_eval = min(EVAL_DAYS, n)
    else:
        n_eval = max(1, int(n * EVAL_RATIO))

    split_idx = n - n_eval
    if split_idx <= 0:
        raise ValueError("Evaluation period too large relative to available samples.")

    # history/context (not evaluated)
    X_hist, y_hist, dates_hist = X[:split_idx], y[:split_idx], dates[:split_idx]

    # final evaluation period
    X_eval, y_eval, dates_eval = X[split_idx:], y[split_idx:], dates[split_idx:]
    ...
    if target_col == "y_next_logret":
        today_close_eval = df["Close"].shift(1).reindex(pd.to_datetime(dates_eval)).values.astype(np.float64)
    ...
    trading_metrics, strat_equity, bh_equity = compute_trading_metrics(today_close_eval, true_close_eval, pred_close_eval)
    ...
    history_dates = pd.to_datetime(dates_hist)
    history_close = df["Close"].loc[history_dates].values

    return {
        "metrics": metrics,
        "history_dates": history_dates,
        "history_close": history_close,
        "eval_dates": pd.to_datetime(dates_eval),
...
def main():
    ...
    result = evaluate_final_period(model, x_scaler, meta, df, device=device)
